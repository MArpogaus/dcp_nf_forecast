...
| `spline_nf_scale_shift` | Normal(0, 1) | Scale(n=48) + Shift(n=48) + RQS(domain=[−4, 4], nbins=12) | 37 | **No results yet** |
| `spline_nf_scale_shift_truncated` | TruncatedNormal(0, 5) | Scale(n=48) + Shift(n=48) + RQS(domain=[0, 5], nbins=8) | 25 | **No results yet** |
...
| `bernstein_nf_scale_shift` | Normal(0, 1) | Scale(n=48) + Shift(n=48) + BernsteinPolynomial(order=8) | 14 | **No results yet** |
| `bernstein_nf_scale_shift_truncated` | TruncatedNormal(0, 5) | Scale(n=48) + Shift(n=48) + BernsteinPolynomial(order=8) | 18 | **No results yet** |
...
| 8 | Add Scale+Shift bijectors, update configs | 4 new models; no evaluation runs yet |
...
| 11 | normal_baseline | −111.62 | 0.1011 | 0.0789 | 1.2419 | 0.001 Cos | [128,128] | 200 |
| 12 | spline_nf_scale_truncated | −102.82 | 0.0247 | 0.0196 | 0.0255 | 0.001 Cos | [256,256] | 400 |
| 13 | bernstein_nf | 42.34 | 0.0511 | 0.0446 | 0.0767 | 0.001 Cos | [256,256] | 400 |

**Winner:** `spline_nf_truncated` (−197.66 NLL, 35 params/dim, MAF + RQS with truncated_normal(0,5) base). Two top models share identical architecture; the constant-LR variant (rank 1, −197.66) substantially outperforms CosineDecay (rank 2, −183.72, −183.72 visible in per-model variation table). Constant LR is strongly preferred on DLA.
...
| 5 | bernstein_nf_scale | −138.86 | 0.0542 | 0.0127 | 0.0169 | 0.0005 | [128,128] | 200 |
| 6 | truncated_baseline | −135.66 | 0.0578 | 0.0168 | 0.0636 | 0.001 | [256,256] | 400 |
| 7 | spline_nf_truncated | −127.05 | 0.1432 | 0.1332 | 0.1751 | 0.0005 | [256,256] | 400 |
| 8 | spline_nf_lognormal (trunc) | −82.66 | 0.1455 | 0.1352 | 0.1712 | 0.0005 | [256,256] | 400 |
| 9 | normal_baseline | −58.13 | 0.0789 | 0.0473 | 0.4597 | 0.001 Cos | [256,256] | 400 |
| 10 | bernstein_nf | 258.07 | 0.1408 | 0.1311 | 0.1603 | 0.0005 Cos | [256,256] | 400 |
...
| 9 | truncated_baseline | −142.00 | 0.0178 | 0.0123 | 0.0806 | 0.001 | [256,256] | 400 |
| 10 | normal_baseline | −131.67 | 0.0278 | 0.0251 | 0.2572 | 0.001 Cos | [256,256] | 400 |
| 11 | bernstein_nf | 184.26 | 0.0660 | 0.0636 | 0.0767 | 0.0005 Cos | [256,256] | 400 |
...
### 4.6 Models Without Evaluation Results

Four models added in Phase 8 have **no evaluation runs** yet:

| Model | Architecture | \#Params/dim | Expected Performance |
|-------|-------------|-------------|---------------------|
| `spline_nf_scale_shift` | Scale + Shift + Spline, Normal(0,1) | 37 | May improve on `spline_nf_scale` by adding location flexibility (Shift) |
| `spline_nf_scale_shift_truncated` | Scale + Shift + Spline, TruncatedNormal(0,5) | 25 | Likely NaN on ofen_g_koks (scale+truncated issue); best candidate for ofen_f_koks |
| `bernstein_nf_scale_shift` | Scale + Shift + Bernstein, Normal(0,1) | 14 | May compete with `bernstein_nf_scale` on pl2 |
| `bernstein_nf_scale_shift_truncated` | Scale + Shift + Bernstein, TruncatedNormal(0,5) | 18 | Unknown; potential for DLA |

Also **re-added without results**: `bernstein_nf_scale_truncated` (config exists, pipeline entry added, no evaluation run).

**To run:**
```bash
source /app/.venv/bin/activate
dvc repro train@dataset<N>-<model>
dvc repro evaluate@dataset<N>-<model>
```
...
6. **Bernstein pure (no scale) is unusable.** `bernstein_nf` with order=8 gives NLL +42 to +288 across all targets. The 8 Bernstein coefficients per dimension produce degenerate densities — likely because the domain [0, 1] is too narrow for the Normal(0,1) base. Adding Shift (Phase 8) may help by allowing the domain to be centred appropriately.
...
2. **Can the Scale+Shift models (Phase 8) close the gap on any target?** The shift bijector may help where the Scale-only variant struggles (DLA, ofen_f_koks).
